...
\noindent Met deze properties kan er een base content schema worden opgezet die properties bevat die in elk content type terugkomen. In \autoref{fig:basecontentjsonschema} staat een voorbeeld van een simpel base content schema die gebruik maakt van de eerder gedefinieerde string property.
...
Het ‘base content’ schema moet gecombineerd kunnen worden met de daadwerkelijke content types die gebruikt zullen worden in de editor en het NGT. Hiervoor maakt JSON-schema gebruik van het ‘allOf’ keyword. Een schema met het ‘allOf’ keyword is pas valide wanneer de onderliggende schema’s, gespecificeerd in ‘allOf’, valide zijn\cite{Droettboom2016}. Het schema in \autoref{fig:jsonschemallofexample} definieert ‘text content’, welke valide is als de twee onderliggende schema’s valide zijn.
...
\begin{figure}[htb]
    ...
    \label{fig:jsonschemallofexample}
\end{figure}
...
In \autoref{fig:representationschematypes} wordt een voorstel gedaan op de representatie van de types\footnote{“null” heeft geen representatie}.
...
\section{Conclusie}
Content typen zorgen voor onderscheid in game content. Het toevoegen van content typen om de diversiteit in game content te ondersteunen kan worden versimpeld door gebruik te maken van JSON-schema’s. Door JSON-schema’s te implementeren in de editors wordt de flexibiliteit van de editors bevorderd. Hiernaast wordt het continu moeten compileren van de editors overbodig.
